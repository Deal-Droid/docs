...
Whether it's budget, pet names, purchase urgency, or any data that matters to your business — AI will extract and remember this information automatically to create a more personalized experience.

---

## Why Define Custom Fields

- 🎯 **Tailored to your business** — Every business has different data needs
- 🧠 **Smarter AI** — The more AI knows about customers, the better and more personalized the responses
- 💾 **No context loss** — Extracted data is saved in customer profiles and available for future conversations
- ✨ **Works alongside default fields** — Doesn't affect basic fields, just add more

> "Truly knowing your customers is the key to successful sales — the more you know, the better you serve"
...
- **Pet name** — Guide: "Customer's pet name (e.g., Brownie, Fluffy, Lucky)"
- **Pet type** — Guide: "Type of pet (e.g., dog, cat, rabbit, bird)"
- **Pet age** — Guide: "Pet's age (e.g., 3 years old, 2 months, 5 years)"
- **Purchase timeline** — Guide: "How urgently the customer needs the product (e.g., urgent, next week, just browsing)"
...
AI: Hello John! Nice to meet Brownie! 🐕
...
- **Budget** — Guide: "Customer's stated budget or spending limit (e.g., 5000 baht, under $100, no budget limit)"
- **Purchase timeline** — Guide: "When the customer needs the product (e.g., urgent, this week, next month, just browsing)"
- **Referral source** — Guide: "How the customer found the business (e.g., friend recommendation, Instagram, Google, TikTok, advertisement)"
...
- **Company or organization** — Guide: "Customer's company name, shop name, or organization they represent"
- **Budget** — Guide: "Customer's stated project budget (e.g., 100,000 baht, $5,000-$10,000)"
- **Purchase timeline** — Guide: "When they need to make the purchase (e.g., this quarter, next year, flexible)"
...
1. **Preferred contact channel** — Customer's preferred way to be contacted beyond current chat (e.g., LINE, WhatsApp, phone call, email)
2. **Budget** — Customer's stated budget or spending limit (e.g., 5000 baht, under $100, no budget limit)
3. **Company or organization** — Customer's company name, shop name, or organization they represent
4. **Purchase timeline** — When the customer needs the product (e.g., urgent, this week, next month, just browsing)
5. **Referral source** — How the customer found the business (e.g., friend recommendation, Instagram, Google, TikTok, advertisement)
...
The name used to identify this field — should be short, concise, and clear
...
- **The clearer the better** — Explain in detail with examples
- **Always include examples using `(e.g., ...)`** — Helps AI understand expected formats
...
- 📝 **Always include examples** → AI will understand expected formats more clearly
- 🎯 **Be specific** → The more detailed, the more accurate AI's extraction
- 🔢 **Specify format for numbers or dates** → e.g., "Amount as a number" or "Date in DD/MM/YYYY format"

When finished, click **Save** or **Add** button

<Aside type="caution">
**Limitation:** You can create a maximum of **10 fields** per Droid. The system will show a counter "X / 10 fields" to show remaining slots.
</Aside>
...
- Old extracted data will **not be deleted** — just stops extracting new data

**Edit field:**

- ⚠️ **No direct edit button**
- To edit name or guide → **Delete and create new**
...
- ✅ **Correct** → AI extracted as expected. Great!
- ❌ **Incorrect** or **Nothing extracted** → Try adjusting Guide to be more specific
...
- Test again → Should not extract the disabled field
...
1. **Saved to Customer Profile** — Combined with other customer data
2. **Stored permanently** — Won't disappear when chat closes or days pass
3. **Used in future responses** — AI will remember and respond more accurately without re-asking
...
> **Benefit:** Customers don't need to repeat information, AI remembers — creates a smooth and personalized experience
...
- Enable only **truly necessary** fields — don't enable everything
- Ask yourself: "Will this data help AI respond better to customers?"
- Fewer fields mean faster and more accurate AI

### 2. Write Good Guides

- ✅ **Always include examples** → e.g., `(e.g., urgent, next week, just browsing)`
- ✅ **Be clear and specific** → "Pet's name" is better than "Pet info"
- ✅ **Specify format** → "Date in DD/MM/YYYY" or "Number without currency symbol"
...
- Delete unused fields → Makes AI work faster
- Don't exceed 10 fields → More fields = more complexity

### 5. When to Disable a Field

- No longer need that data
- AI extracts incorrectly often → Try adjusting Guide first, disable if still not working
...
<Aside type="note">
**AI extracts data from conversations only — It doesn't ask customers**
...
- If customers **don't mention** the data → AI **won't extract anything**
...
|----------|---------------------------|------------------------|---------------------------|
| **Does AI ask?** | ❌ Doesn't ask | ✅ Asks only in specific Intents | ✅ Asks in all Intents |
| **Extracts when customer speaks?** | ✅ Always | ✅ Always | ✅ Always |
...
> **Important:** Regardless of approach, **Extraction Fields always auto-extract** when customers mention data — even if AI doesn't ask
...
- ✅ Is the field **enabled**? (toggle = ON)
- ✅ Is the Guide **clear enough**? → Try adding examples
- ✅ Does the customer's message **contain relevant data**?
...
- Need **at least 1 field with a value** → If none, section won't appear
- Don't forget to **click the `i` button** to flip the card

### Want to Edit Field Name or Guide

- ⚠️ **No direct edit**
- **Solution:** Delete old field → Create new field
...
<Aside type="tip">
This section is for users who want to understand how things work behind the scenes — **not required reading for normal use**
</Aside>
...
- **No type casting** — Guide tells AI what type of data to extract

### Performance

- AI uses **parallel extraction** → Doesn't add latency
- If all fields disabled → AI **skips processing** immediately (zero overhead)
- Fewer fields = faster AI

### Deleting Fields

- Deleting fields **doesn't delete old data** → Orphaned data remains in customer profiles
- Just stops new extraction
- Old data remains usable (safe, no data loss)

### Validation

- Field names must be unique (**case-insensitive**) → "Budget" = "budget" = "BUDGET"
- Validation works on both UI and server
- If AI returns invalid data → System skips it and logs warning (doesn't block pipeline)
